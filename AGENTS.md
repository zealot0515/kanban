...
- Launch profile schemas must stay dependency-free from `src/core/api-contract.ts`; the API contract imports them, so importing the contract back into `src/core/launch-profiles.ts` creates a runtime initialization cycle.
- Saved launch profile values are intentionally returned only as `{ name, configured }` summaries. The encrypted profile file is global to the user, while task-level environment overrides remain in the task data and are unencrypted.
- Passing `OPENAI_API_KEY` to Codex does not select a custom provider or replace ChatGPT authentication. CLIProxy launch profiles must explicitly override `model_provider`, the provider's `base_url`/`env_key`, and `requires_openai_auth=false` through argv. Keep credentials in the encrypted variables and process environment, never argv or the user's Codex config/auth files. Verify routing with a local mock Responses endpoint and dummy keys, since checking the spawned environment alone misses this failure.
- CLI arguments have explicit command and literal-line input modes. Preserve legacy `cliArgs` arrays verbatim; never shell-parse already-tokenized TOML strings. `cliArgsInput` retains editable text (including trailing newlines and unfinished quotes) until validation, and takes precedence over legacy argv. Do not use `split/filter/join` as a controlled textarea's editing state: it eats Enter/trailing newlines. Before diagnosing provider settings, verify argv grouping: a shell-style `-c 'key=value'` paste was previously passed as one argument, and bare provider assignments cause Codex's `unexpected argument` error. Manual `-c` flags can fully configure CLIProxy without the provider editor.
- Blank-prompt Start must create a real task card and open its detail view, using the normal task session/worktree/hooks path. Keep its prompt empty (a default title is display-only), preserve titled empty cards in board normalization, and suppress startup images/plan input. Do not route it to the synthetic home terminal; users need the task page to configure the CLI before their first message.
- The Cline SDK transitively installs `@openai/codex` via `ai-sdk-provider-codex-cli`, creating `node_modules/.bin/codex`. Launching the dev runtime through `npm run` prepends that directory to PATH and shadows the user's installed CLI. The custom dev shell must invoke `node scripts/dev-full.mjs` directly. In a prior macOS malware alert, system logs identified this bundled Codex, while the Homebrew binary passed signature verification; always identify the actual executable path before diagnosing the user's global installation.
- Label filtering must retain the full board for mutations: drag-and-drop emits visible indices, while `applyDragResult` expects full-column indices. Translate both positions, including no-op drops. Bulk start must pass visible task IDs through `useTaskStartActions` while checking dependencies against the full board, since hidden tasks can still block a visible task.
- xterm's default WebLinksAddon and OSC 8 handlers open a blank window before assigning its URL. Electron's window-open guard rejects that blank URL, so terminal links silently fail in desktop builds. Keep both handlers wired to `openTerminalLink`, which opens the final HTTP(S) URL directly; do not loosen the desktop guard to allow blank windows.
- Codex task launches use native `codex-hook` callbacks, not the legacy `codex-wrapper` rollout watcher. PermissionRequest precedes guardian approval; `permission_mode` cannot identify automatic review, but the exact transcript's current `turn_context.approvals_reviewer` can. Keep automatic review In Progress even before PostToolUse; only a pending human approval may resume on Enter plus a TUI prompt repaint (never an idle completed turn). Root PreToolUse/PostToolUse restore In Progress. Only root Stop completion may read final-answer fallback, scoped to its session/turn; all `source.subagent` variants (including `{ other: "guardian" }`) must be excluded, not just `thread_spawn`. Preserve hook metadata through `parseHookIngestRequest`, including explicit null to clear an old final message, and apply it before asynchronous checkpoint capture.
