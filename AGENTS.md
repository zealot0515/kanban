...
- The blank-prompt Start action reuses the synthetic `__home_terminal__` session. Stop an existing shell session before replacing it with an interactive agent CLI, otherwise `TerminalSessionManager` correctly returns the already-running process.
